...
class Tag
{
    ...
    public function getByTag($tags)
    {
        ...
        $post_ids = $this->getPostIdsWithAllTags($tags);

        // Step 2: Fetch posts with all tags
        return $this->getPostsWithAllTags($post_ids);
    }

    private function getPostIdsWithAllTags($tags)
    {
        $tag_count = count($tags);
        $placeholders = implode(',', array_fill(0, $tag_count, '?'));

        // Query to fetch post IDs that contain all tags in the $tags array
        $query = "SELECT post_id 
                  FROM " . $this->blog_post_tags_table . " bpt 
        ...
        // Bind the tags and the tag count for the HAVING clause
        foreach ($tags as $index => $tag) {
            $stmt->bindValue(($index + 1), $tag);
        }
        $stmt->bindValue(($tag_count + 1), $tag_count, PDO::PARAM_INT);
